...
func round2(i uint64, ski *big.Int, pk Point, nonce Nonce, msg []byte, cs []Commit) *big.Int {
	...
	lambda_i_num, lambda_i_den := deriveInterpolatingValue(i, members)
	...
	lski := new(big.Int).Mul(lambda_i_num, ski)
	lskii := new(big.Int).Div(lski, lambda_i_den)
	lskic := new(big.Int).Mul(lskii, challenge)
...
func verifySignatureShare(
	...
	cli := new(big.Int).Mul(challenge, lambda_i_num)
	cli.Div(cli, lambda_i_den)
